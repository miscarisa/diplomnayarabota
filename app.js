(() => {
  ...
  const ui = {
    ru: {
      locale: 'RU-RU',
      titlePrefix: 'Протокол № ',
      nav: ['Вывоз за границу', 'Получить инструкцию', 'Протоколы исследований'],
      exportMenu: ['Создать заявку', 'Поиск заявки'],
      home: 'Главная',
      sections: ['Владелец животного', 'Сведения о животном', 'Результат исследования'],
      labels: [
        'ФИО', 'Контактный телефон', 'E-mail',
        'Вид животного', 'Порода', 'Пол', 'Кличка', 'Дата рождения', 'Сведения об идентификации', 'Вакцинации от бешенства', 'Дата отбора крови', 'Место отбора крови',
        'Протокол исследований', 'Статус протокола', 'Номер экспертизы', 'Дата экспертизы', 'Результат исследования'
      ],
      status: 'Оформлен',
      protocolDateJoiner: ' от ',
    ...
    en: {
      locale: 'EN-US',
      titlePrefix: 'Protocol No. ',
      nav: ['Export abroad', 'Get instructions', 'Research protocols'],
      exportMenu: ['Create application', 'Search application'],
      home: 'Home',
      sections: ['Animal owner', 'Animal details', 'Research result'],
      labels: [
        'Full name', 'Contact phone', 'E-mail',
        'Animal species', 'Breed', 'Sex', 'Name', 'Date of birth', 'Identification', 'Rabies vaccination', 'Blood sampling date', 'Blood sampling place',
        'Research protocol', 'Protocol status', 'Examination number', 'Examination date', 'Research result'
      ],
      status: 'Completed',
      protocolDateJoiner: ' dated ',
  ...
  function applyLanguage(lang) {
    ...
    document.querySelectorAll('[data-field]').forEach((node) => {
      ...
      node.textContent = fieldValue(key, lang);
    });

    const localeButton = document.getElementById('localeButton');
    if (localeButton) localeButton.textContent = t.locale;

    const toolbarNav = [
      document.querySelector('#exportButton span'),
      document.querySelectorAll('.tool-link span')[0],
      document.querySelectorAll('.tool-link span')[1]
    ];
    toolbarNav.forEach((el, i) => { if (el) el.textContent = t.nav[i]; });

    const drawer = document.querySelectorAll('.mobile-drawer a');
    drawer.forEach((el, i) => setTextPreserveIcon(el, t.nav[i]));

    document.querySelectorAll('.export-menu a').forEach((el, i) => {
      if (t.exportMenu[i]) el.textContent = t.exportMenu[i];
    });

    const crumbHome = document.querySelector('.crumbbar > a');
    if (crumbHome) crumbHome.textContent = t.home;
    const crumbProtocol = document.querySelector('.crumbbar > span:last-child');
    if (crumbProtocol && crumbProtocol.firstChild) crumbProtocol.firstChild.nodeValue = t.titlePrefix;

    const h1 = document.querySelector('.page h1');
    if (h1 && h1.firstChild) h1.firstChild.nodeValue = t.titlePrefix;

    document.querySelectorAll('.protocol-section h2').forEach((el, i) => {
      if (t.sections[i]) el.textContent = t.sections[i];
    });
    ...
    if (protocolRowValue) {
      protocolRowValue.innerHTML = `<span data-field="protocolNumber">${raw.protocolNumber}</span>${t.protocolDateJoiner}<span data-field="protocolDate">${raw.protocolDate}</span>`;
    }

    const status = document.querySelector('.result-section .row:nth-child(2) .value');
    ...
    if (mobileButton) mobileButton.setAttribute('aria-label', t.mobileMenuAria);
    const backTop = document.getElementById('backTop');
    if (backTop) backTop.setAttribute('aria-label', t.backTopAria);

    const emailLink = document.getElementById('ownerEmail');
    if (emailLink) emailLink.href = `mailto:${raw.email}`;
  ...
  const localeLinks = document.querySelectorAll('#localeMenu a');
  localeLinks.forEach((link) => {
    link.addEventListener('click', (event) => {
      event.preventDefault();
      applyLanguage(link.textContent.trim().startsWith('EN') ? 'en' : 'ru');
      closeAll();
    });
  });
